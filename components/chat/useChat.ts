    ...
    async (text: string, settings: SendSettings = {}) => {
      ...
      try {
        ...
        if ((e as Error).name !== "AbortError") {
          ...
          // A 409 session_busy means the previous turn is still wrapping up — soften it.
          setError(
            /busy|in flight|already running/i.test(message)
              ? "Your agent is still finishing the previous message. Give it a moment and try again."
              : message
          );
        }
      } finally {
