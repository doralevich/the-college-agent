...
// drop zone. Shared by the chat composer and the files pane. dragenter/dragleave fire for every
// child the cursor crosses and bubble up to the (large) zone, so we count enters minus leaves
// rather than inspecting relatedTarget — the latter is null on Safari/Firefox, which would
// strobe the overlay on each child boundary crossing.
